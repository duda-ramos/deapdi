...
export const mentalHealthService = {
  ...
  async getWellnessResourcesWithStats(category?: string): Promise<WellnessResource[]> {
    ...
    return supabaseRequest(() => query.order('created_at', { ascending: false }), 'getWellnessResourcesWithStats');
  },

  async recordResourceView(resourceId: string, userId: string, duration = 0): Promise<void> {
    console.log('🧠 MentalHealth: Recording resource view');
  ...
  // Therapeutic Tasks Management
  async getTherapeuticTasks(userId?: string): Promise<any[]> {
    ...
    let query = supabase
      ...
      .select(`
        *,
        assigned_by_user:profiles!assigned_by(id, name, avatar_url)
      `);
    ...
    return supabaseRequest(() => query.order('created_at', { ascending: false }), 'getTherapeuticTasks');
  },

  async createTherapeuticTask(task: any): Promise<any> {
    console.log('🧠 MentalHealth: Creating therapeutic task');
  ...
  async updateTherapeuticTask(id: string, updates: any): Promise<any> {
  ...
  async completeTherapeuticTask(id: string, completionNotes: string, effectivenessRating?: number): Promise<any> {
  ...
  async deleteFormTemplate(id: string): Promise<void> {
    console.log('🧠 MentalHealth: Deleting form template:', id);

    return supabaseRequest(() => supabase
      .from('form_templates')
      .delete()
      .eq('id', id), 'deleteFormTemplate');
  },
  ...
  async deleteAlertRule(id: string): Promise<void> {
    console.log('🧠 MentalHealth: Deleting alert rule:', id);

    return supabaseRequest(() => supabase
      .from('alert_rules')
      .delete()
      .eq('id', id), 'deleteAlertRule');
  },

  // Enhanced Analytics
  async getMentalHealthAnalytics(startDate?: string, endDate?: string): Promise<any> {
    console.log('🧠 MentalHealth: Getting enhanced analytics');
    ...
    try {
      ...
      console.warn('🧠 MentalHealth: Error getting analytics, returning fallback data:', error);
      return {
  ...
  async getFavoriteResources(userId: string): Promise<WellnessResource[]> {
    ...
    return result.map((item: any) => item.resource);
  },
  ...
  getFormTypeLabel(type: string): string {
    switch (type) {
      case 'auto_avaliacao': return 'Auto-avaliação';
      case 'feedback_gestor': return 'Feedback do Gestor';
      case 'avaliacao_rh': return 'Avaliação RH';
  ...
  getTaskStatusBadge(status: string): 'default' | 'warning' | 'success' | 'danger' {
    switch (status) {
