-- ============================================================================
-- VALIDAÇÃO FINAL DE PROTEÇÃO DE DADOS ULTRA-SENSÍVEIS (CORRIGIDA)
-- ============================================================================
-- Objetivo: Confirmar que dados sensíveis estão 100% protegidos
-- Data: 2025-11-25
-- Versão: 1.1 - Corrigida para verificar existência de tabelas
-- ============================================================================
...
-- ============================================================================
-- PARTE 0: VERIFICAR QUAIS TABELAS SENSÍVEIS EXISTEM
-- ============================================================================

\echo '========================================='
\echo 'PARTE 0: VERIFICAR TABELAS EXISTENTES'
\echo '========================================='
\echo ''

\echo 'Tabelas sensíveis - Status de existência:'
\echo ''

WITH sensitive_tables_list AS (
  SELECT unnest(ARRAY[
...
SELECT 
  stl.table_name as "Tabela Sensível",
  CASE 
    WHEN t.tablename IS NOT NULL THEN '✅ EXISTE'
    ELSE '⚠️ NÃO EXISTE'
  END as "Status",
  CASE 
    WHEN t.tablename IS NULL THEN 'Ignorar validação'
    ELSE 'Validar RLS'
  END as "Ação"
FROM sensitive_tables_list stl
LEFT JOIN pg_tables t ON t.tablename = stl.table_name AND t.schemaname = 'public'
ORDER BY 
  CASE WHEN t.tablename IS NOT NULL THEN 1 ELSE 2 END,
  stl.table_name;

\echo ''

-- ============================================================================
-- PARTE 1: CONTAR POLÍTICAS DE TABELAS SENSÍVEIS (SOMENTE AS QUE EXISTEM)
-- ============================================================================
...
\echo '1. Contagem de Políticas por Tabela Sensível (apenas tabelas existentes):'
\echo ''
...
existing_tables AS (
  ...
  JOIN pg_tables t ON t.tablename = st.table_name AND t.schemaname = 'public'
)
SELECT 
  p.tablename as "Tabela Sensível",
...
-- ============================================================================
-- PARTE 2: VERIFICAR RLS HABILITADO (SOMENTE TABELAS EXISTENTES)
-- ============================================================================
...
\echo '2. Status de RLS nas Tabelas Sensíveis (apenas existentes):'
\echo ''
...
AND t.tablename IN (SELECT table_name FROM sensitive_tables)
...
-- 3.1 PSYCHOLOGICAL_RECORDS
\echo '3.1 - PSYCHOLOGICAL_RECORDS (Registros Psicológicos):'
\echo ''

DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'psychological_records') THEN
    PERFORM 1; -- Tabela existe, queries abaixo irão executar
  ELSE
    RAISE NOTICE '⚠️ Tabela psychological_records não existe - pulando validação';
  END IF;
END $$;
...
SELECT 
  ...
  CASE 
    WHEN EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'psychological_records')
    THEN COUNT(*)
    ELSE NULL
  END as "Políticas com manager",
  CASE 
    WHEN NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'psychological_records')
    THEN 'ℹ️ Tabela não existe'
    WHEN COUNT(*) > 0 THEN '❌ VULNERABILIDADE - Manager tem acesso!'
    ELSE '✅ OK - Manager bloqueado'
  END as "Status"
FROM pg_policies
...
-- ============================================================================
-- PARTE 6: RESUMO EXECUTIVO (CORRIGIDO)
-- ============================================================================
...
existing_tables AS (
  ...
  WHERE EXISTS (
    SELECT 1 FROM pg_tables t
    WHERE t.tablename = st.table_name
...
rls_status AS (
  ...
  FROM existing_tables et
  LEFT JOIN pg_tables t ON t.tablename = et.table_name
...
SELECT 
  COUNT(*) as "Total de Tabelas Sensíveis Existentes",
...
BEGIN
  ...
  IF v_protected = v_total THEN
    ...
    RAISE NOTICE '';
    RAISE NOTICE '🎉 TODAS AS % TABELAS SENSÍVEIS EXISTENTES ESTÃO 100%% PROTEGIDAS!', v_total;
    RAISE NOTICE '';
...
\echo ''
\echo 'NOTA: therapy_session_requests não existe no banco - validação ignorada'
\echo ''
